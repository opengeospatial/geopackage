...
.. If the integer representation of the user_version string is less than 10300, fall back to the tests for GeoPackage 1.2.
...
[cols="1,5a"]
|========================================
|*Test Case ID* |[line-through]#+/opt/valid_geopackage+#
|*Test Purpose* |[line-through]#Verify that a GeoPackage contains a features or tiles table and `gpkg_contents` table row describing it.#
|*Test Method* |
. [line-through]#SELECT COUNT(*) FROM gpkg_contents WHERE data_type IN ('tiles', 'features')#
. [line-through]#Pass if result > 0#
. [line-through]#Fail otherwise#
|*Reference* |[line-through]#Clause 2 Req 17#:
|*Test Type* |[line-through]#Capability#
|========================================
...
... Fail if cn.flags.E is not in the set {0, 1, 2, 3, 4}
.  Pass if no fails
...
[cols="1,5a"]
|========================================
|*Test Case ID* |+/opt/features/geometry_encoding/data/empty_geometry+
|*Test Purpose* |Verify that empty geometries are encoded consistently in the StandardGeoPackageBinary format.
|*Test Method* |
. SELECT table_name AS tn, column_name AS cn FROM gpkg_geometry_columns WHERE table_name IN (SELECT table_name FROM gpkg_contents WHERE data_type = 'features')
. Not testable if returns an empty result set
. For each row from step 1
...
.. Not testable if none found
.. For each cn value from step a
... Fail if gc.flags.Y == 1 and gc.flags.E != 0 
... Fail if the geometry is empty but the envelope is not empty (gc.flags.E != 0 and envelope values are not NaN)
.  Pass if no fails
|*Reference* |Clause 2.1.3.1.1 Req 152:
|*Test Type* |Basic
|========================================
