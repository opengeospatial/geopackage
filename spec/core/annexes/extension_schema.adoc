...
|`name` |TEXT |A human-readable name for the column_name content |yes |UK
...
[NOTE]
=====
`gpkg_data_columns.name` contains a name (also called a label or a title) for an attribute type, and is therefore the equivalent of `gpkg_contents.identifier`, which provides a name for an entity type. For historical reasons, these columns have a different name. `gpkg_data_columns.title` does not have an equivalent in `gpkg_contents`.
=====
