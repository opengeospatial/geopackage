...
The indexes provided by the SQLite Virtual Table RTree extension are not automatic indexes.
...
/* Conditions: Update of geometry column to non-empty geometry
               when the old geom is neither null nor empty
...
/* Conditions: Update of geometry column to non-empty geometry
               when the old geom is null/empty
   Actions   : Update record in rtree */
...
GeoPackage Version 1.3.0 fixes this issue.
...
[WARNING]
====
GeoPackage Versions 1.3.1 and prior have a single update1 trigger that is incompatible with upsert statements.
GeoPackage Version 1.4.0 fixes this issue.
For older GeoPackages where upsert compatibility is desired, a new extension is being established that mimics the 1.4.0 behavior.
====
...
[NOTE]
====
The minimum bounding indexes created within the RTree Extension for GeoPackage should reflect the appropriate bounding area for the indexed feature.
...
Clients should exercise care when using these indexes during queries because the SQLite RTree module might link:https://www.sqlite.org/rtree.html#roundoff_error[round bounding boxes slightly outward] (up to 0.000012%).
