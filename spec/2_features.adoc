...
:min_runtime_foot2: footnote:[SQL functions on geometries in addition to those defined in this clause should conform to the SF/SQL <<N14>><<N15>> and SQL/MM <<N16>> specifications cited in clause <<sfsql_intro>> above.
...
SQL/MM <<N16>> <<sqlmm_gpkg_geometry_columns_sql>> and OGC Simple Features SQL <<N14>><<N15>>
...
:features_data_table_foot1: footnote:[A feature type MAY be defined to have 0..n geometry and/or raster attributes, so the corresponding feature table MAY contain from 0..n geometry and/or raster columns.]
...
geometries. {features_data_table_foot1} Features are rows in a feature table. {features_data_table_foot2}
