...
image::spec/annexes/images/umlnotationforrdbmstables.png[]
...
b. \<\> An instantiation of a UML attribute as an RDBMS table column.
...
image::spec/annexes/images/umlnotationforxmlschema.png[]

In this standard, the following stereotypes of UML classes are used to describe XML schemas:

a. \<\> A descriptor of a set of values that lack identity (independent existence and the possibility of side effects). A DataType is a class with no operations whose primary purpose is to hold the information.
b. \<\> is a fixed enumeration that uses string values for expressing a list of potential values.
c. \<\> is an extensible enumeration that uses string values for expressing a list of potential values.
