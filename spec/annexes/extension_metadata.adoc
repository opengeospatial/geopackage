...
GeoPackages complying with this extension SHALL have rows in the `gpkg_extensions` table as described in <<ExtensionTableRecord>> (below).
====
...
. Fail if both table_name entries are not present
. Pass if no fails
