...
.Requirement 92
====
(extends <<r36>> and <<r37>>) A GeoPackage that contains a tile pyramid user data table that contains tile data MAY store tile_data in the WebP format<<I22>>. Files complying with the WebP format SHALL have the http://www.ietf.org/rfc/rfc2046.txt[MIME type] `image/x-webp`.
====
