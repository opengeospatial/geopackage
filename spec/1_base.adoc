...
In addition, some RDBMS systems truncate identifier names that are longer than a fixed byte length with may be as short as 30 bytes.
Therefore, long GeoPackage identifier values SHOULD be avoided and if long identifer values are used, then they SHOULD be unique for at least the first 30 bytes.
====
