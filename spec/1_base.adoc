...
:req2_foot1: footnote:[With SQLite versions 3.7.17 and later this value MAY be set with the "PRAGMA application_id=1196437810;" SQL statement, where 1196437810 is the 32-bit integer value of 0x47503132. With earlier versions of SQLite the application id can be set by writing the byte sequence 0x47, 0x50, 0x31, 0x32 at offset 68 in the SQLite database file (see http://www.sqlite.org/fileformat2.html#database_header for details).]
:req2_foot2: footnote:[Applications should be aware of the possibility that newer versions of GeoPackage may be published and that the application may have a higher suffix than "12".]
...
[requirement]
A GeoPackage SHALL contain an appropriate value in the application ID field of the SQLite database header to indicate that it is a GeoPackage. {req2_foot1} The value SHALL have a prefix of 0x4750 ("GP" in ASCII). A value of 0x47503132 ("GP12" in ASCII) SHALL indicate a GeoPackage version 1.2 file. {req2_foot2}
...
The http://www.sqlite.org/download.html[SQLite] <<8>> library has many http://www.sqlite.org/compile.html[compile time] and http://www.sqlite.org/pragma.html[run time] options that MAY be used to configure SQLite for different uses. Certain elements of the GeoPackage specification depend on the availability of SQLite functionality at runtime. This clause specifies the set of compile  time options that SHALL or SHALL NOT be used.
