...
|DATE                 | ISO-8601 date string in the form YYYY-MM_DD encoded in either UTF-8 or UTF-16. See TEXT. Stored as SQLite TEXT
...
The http://www.sqlite.org/download.html[SQLite] <<8>> library has many http://www.sqlite.org/compile.html[compile time] and http://www.sqlite.org/pragma.html[run time] options that MAY be used to configure SQLite for different uses.
