...
.Requirement 148
====
When the geometry envelope of a GeoPackageBinary is not empty and valid, the geometry envelope for each feature instance SHALL fall within the extents of the feature table as expressed the corresponding entry in `gpkg_geometry_columns`.
====
