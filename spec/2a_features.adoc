...
In a GeoPackage, “simple” features are geolocated using a linear geometry subset of the SQL/MM (ISO 13249-3) <<16>> geometry model shown in <<core_geometry_model_figure>> below.
...
<6> per  ISO 13249-3 <<16>> clause 5.1.46 {geopackage_binary_foot1}
...
:geom_type_req_foot2: footnote:[GeoPackage applications MAY use SQL triggers or tests in application code to further restrict the geometry types that are allowed in a feature table, for example to allow only points, or prohibit heterogeneous GEOMCOLLECTIONs.]
...
If the geometry `type_name` is GEOMETRY (the root of the geometry type hierarchy) then the feature table geometry column MAY contain geometries of any geometry type. {geom_type_req_foot2}
