...
International specifications <<13>><<14>><<15>><<16>> have standardized practices for the storage, access and use of vector geospatial features and geometries via SQL in relational databases.
...
In a GeoPackage file, “simple” features are geolocated using a linear geometry subset of the SQL/MM (ISO 13249-3) <<16>> geometry model shown in <<core_geometry_model_figure>> below.
...
A more detailed description can be found in ISO 13249-3 <<16>>.
...
[requirement]
A GeoPackage file SHALL store feature table geometries with or without optional elevation (Z) and/or measure (M) values in SQL BLOBs using the GeoPackageBinary format specified in table <<gpb_spec>> and clause <<gpb_format>>.
...
<5>  per  ISO 13249-3 <<16>> clause 5.1.46 {geopackage_binary_foot1}

Well-Known Binary as defined in ISO 13249-3 <<16>> does not provide a standardized encoding for an empty point set (i.e., 'Point Empty' in Well-Known Text).
...
:min_runtime_foot2: footnote:[SQL functions on geometries in addition to those defined in this clause SHOULD conform to the SF/SQL <<13>><<14>><<15>> and SQL/MM <<16>> specifications cited in clause <<sfsql_intro>> above.]
...
One that does so SHALL provide the minimal runtime SQL functions listed in <<minimal_runtime_sql_functions>> table <<minimal_runtime_sql_functions_table>>.
...
Views of this table or view MAY be used to provide compatibility with the SQL/MM <<16>> <<sqlmm_gpkg_geometry_columns_sql>> and OGC Simple Features SQL <<13>><<14>><<15>> <<sfsql_gpkg_geometry_columns_sql>> specifications.
